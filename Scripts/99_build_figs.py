...
def fig_estimator_mix(df_diag, out_dir):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["Thrust","Torque","Speed","Vibration"])
    ax.set_ylabel("Count")
...
def fig_vibration_vs_eta_var(df_diag_or_joined, df_master, out_dir):
    ...
    ax.set_ylabel("Figure of merit variance")
...
def fig_pareto_contribs(df_opt, out_dir):
    ...
    ax.plot(d["w_hover"], d["explore_hover"], color="0.1", linestyle="--", label="Hover explore")
    ax.plot(d["w_hover"], d["exploit_cruise"], color="0.6", linestyle="-",  label="Cruise exploit")
    ax.plot(d["w_hover"], d["explore_cruise"], color="0.6", linestyle="--", label="Cruise explore")
    ax.set_xlabel("Hover weight")
...
def fig_pareto_all_stacked(df_opt,
                           df_base_sweep,
                           out_dir,
                           sections="1-9",
                           fname="Fig_Pareto_Stacked.pdf"):
    """
    Build a vertical, shared-x figure with any subset of panels [1..9].

    Panels (by index):
      [1] UCB vs w (optimized; optional baseline envelope if df_base_sweep provided)
      [2] Contributions (hover/cruise exploit/explore)
      [3] Geometry vs w: AR
      [4] Geometry vs w: lambda
      [5] Geometry vs w: twist
      [6] Operations vs w: alpha_hover
      [7] Operations vs w: v_hover
      [8] Operations vs w: alpha_cruise
      [9] Operations vs w: v_cruise

    Args
    ----
    df_opt : DataFrame with columns:
        w_hover, ucb_total, exploit_hover, explore_hover, exploit_cruise, explore_cruise,
        AR, lambda, twist, alpha_hover, v_hover, alpha_cruise, v_cruise
        (only the columns for selected panels are required)
    df_base_sweep : DataFrame or None
        If provided and contains w_hover + ucb_total, panel [1] shows a baseline envelope.
    out_dir : str
        Output directory for the PDF.
    sections : str | Iterable[int]
        e.g., "1,2,3-5,6-9" or [1,2,6,7]. Defaults to "1-9".
    fname : str
        Output filename (PDF).

    Returns
    -------
    None (saves out_dir/fname)
    """
    import os
    import numpy as np
    import pandas as pd
    ...
    panels = _parse_sections(sections)
    if not panels:
        return
    ...
    have_base = (
        df_base_sweep is not None
        and not df_base_sweep.empty
        and {"w_hover", "ucb_total"}.issubset(df_base_sweep.columns)
    )

    # Panel availability by required columns
    required = {
        1: {"ucb_total"},
        2: {"exploit_hover","explore_hover","exploit_cruise","explore_cruise"},
        3: {"AR"},
        4: {"lambda"},
        5: {"twist"},
        6: {"alpha_hover"},
    ...
    panels = [p for p in panels if required.get(p, set()).issubset(set(do.columns))]
    if not panels:
        return

    # ---------- figure ----------
    height_per_row = 1.2  # inches per panel (adjust if labels are crowded)
    fig, axes = plt.subplots(len(panels), 1,
                             figsize=(3.4, height_per_row*len(panels)),
                             sharex=True)
    if len(panels) == 1:
        axes = [axes]
    axes = np.asarray(axes)
    ...
    def _grid(ax): ax.grid(True, color="0.9", linewidth=0.6, alpha=0.8)

    # ---------- plotting helpers per panel ----------
    def _panel1(ax):
        # UCB vs w with optional baseline envelope
        if have_base:
            db = df_base_sweep.copy()
            grp = db.groupby("w_hover")
            w_vals = sorted(grp.groups.keys())
            ...
            ax.fill_between(w_vals, base_lo, base_hi, color="0.85", alpha=0.6, linewidth=0)
            ax.plot(w_vals, base_med, color="0.55", linestyle="--", linewidth=1.0, label="Baseline median")
        ax.plot(w, do["ucb_total"], color=greys[1], linewidth=1.2, label="Optimized")
    ...
    def _panel2(ax):
        ...
        ax.plot(w, do["exploit_cruise"], color="0.3", linestyle="-",  label="Cruise exploit")
        ax.plot(w, do["explore_cruise"], color="0.7", linestyle="--", label="Cruise explore")
        ax.set_ylabel(ylabels[2]); _grid(ax)
        ax.legend(frameon=False, ncols=2, loc="best")

    def _panel_geom(ax, col, idx):
        ax.plot(w, do[col], color=greys[idx], linewidth=1.2)
        ax.set_ylabel(ylabels[idx]); _grid(ax)
        if col == "lambda":
            ax.yaxis.set_major_locator(mpl.ticker.MaxNLocator(6))
            ax.yaxis.set_major_formatter(mpl.ticker.FormatStrFormatter("%.2f"))

    def _panel_ops(ax, col, idx):
        ax.plot(w, do[col], color=greys[idx], linewidth=1.2)
        ax.set_ylabel(ylabels[idx]); _grid(ax)

    # Dispatcher
    draw = {
        1: lambda ax: _panel1(ax),
        2: lambda ax: _panel2(ax),
        3: lambda ax: _panel_geom(ax, "AR", 3),
        4: lambda ax: _panel_geom(ax, "lambda", 4),
        5: lambda ax: _panel_geom(ax, "twist", 5),
        6: lambda ax: _panel_ops(ax, "alpha_hover", 6),
        7: lambda ax: _panel_ops(ax, "v_hover", 7),
        8: lambda ax: _panel_ops(ax, "alpha_cruise", 8),
        9: lambda ax: _panel_ops(ax, "v_cruise", 9),
    }
...
def main():
    ...
    # Only [1] and [2]
    #fig_pareto_all_stacked(df_opt, df_base_sweep, plots_dir, sections="1,2")

    # Geometry only [3–5]
    #fig_pareto_all_stacked(df_opt, df_base_sweep, plots_dir, sections="3-5", fname="Fig_Pareto_Geom_vs_w.pdf")

    # Operations only [6–9]
    #fig_pareto_all_stacked(df_opt, df_base_sweep, plots_dir, sections=[6,7,8,9], fname="Fig_Pareto_Ops_vs_w.pdf")

    # Mixed: [1], [3–5], [6–7]
    fig_pareto_all_stacked(df_opt, df_base_sweep, plots_dir, sections="3-9")
