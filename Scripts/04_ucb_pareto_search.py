...
def _calc_global_uncertainty(model, df_mode: pd.DataFrame, median_perf: float, features: list[str], n_samples=10000, eps=1e-9) -> float:
    """
    ...
    med_mu = float(np.median(mu))
    mean_s = float(np.mean(s))
    denom = med_mu if med_mu > eps else max(abs(med_mu, eps))
    return 100.0 * mean_s / denom
