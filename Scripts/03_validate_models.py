...
def _lambda_calibration(y, mu, std, meas_var=None):
    ...
    if r.size == 0:
        return np.nan, np.nan, np.nan, np.nan
    q68 = float(np.quantile(r, 0.68))
    q95 = float(np.quantile(r, 0.95))
    lam68 = q68 / 1.0     # since z_68 = 1.0
    lam95 = q95 / 1.96
    return q68, q95, lam68, lam95
...
def main():
    ...
    if not df_hover.empty:
        ...
        q68_h, q95_h, lam68_h, lam95_h = _lambda_calibration(y_true, y_pred, y_std, meas_var=m_var)
    ...
    if not df_cruise.empty:
        ...
        q68_c, q95_c, lam68_c, lam95_c = _lambda_calibration(y_true, y_pred, y_std, meas_var=None)

    if metrics_rows:
        pd.DataFrame(metrics_rows).to_csv(tables_dir / "03_loocv_metrics.csv", index=False)
        # Write calibration table with both quantiles and lambdas
        cal_rows = []
        if np.isfinite(lam95_h):
            cal_rows.append({"mode":"hover","q_68":q68_h,"q_95":q95_h,"lambda_68":lam68_h,"lambda_95":lam95_h})
        if np.isfinite(lam95_c):
            cal_rows.append({"mode":"cruise","q_68":q68_c,"q_95":q95_c,"lambda_68":lam68_c,"lambda_95":lam95_c})
